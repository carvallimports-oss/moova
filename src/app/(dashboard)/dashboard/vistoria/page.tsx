    ...
    <div className="p-6 lg:p-8 pt-20 lg:pt-8 max-w-3xl mx-auto space-y-6">
      <div>
        <div className="flex items-center gap-2 mb-1">
          <ClipboardList className="w-5 h-5 text-[#787F56]" />
          <h1 className="font-serif text-2xl text-[#30360E]">Moova Vistoria</h1>
        </div>
        <p className="text-sm text-[#7A7A6A]">Gere laudos de vistoria profissionais com apoio da IA.</p>
      </div>

      <div className="bg-white border border-[#D4C5A0] rounded-xl p-6 space-y-5">
        <h2 className="font-serif text-lg text-[#30360E]">Nova vistoria</h2>
        ...
        <div className="space-y-3">
          <div className="flex items-center justify-between">
            <label className="text-xs font-medium text-[#4A4A3A]">Cômodos *</label>
            <button onClick={addRoom} className="text-xs text-[#30360E] flex items-center gap-1 hover:underline">
              <Plus className="w-3 h-3" /> Adicionar cômodo
            </button>
          </div>
          {rooms.map((room, idx) => (
            <div key={idx} className="bg-[#F5F0E0] rounded-lg p-3 space-y-2">
              <div className="flex gap-2">
                <input
                  value={room.name}
                  onChange={(e) => updateRoom(idx, "name", e.target.value)}
                  placeholder="Nome do cômodo"
                  className="flex-1 border border-[#D4C5A0] rounded-lg px-3 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#30360E]/20"
                />
                <select
                  value={room.condition}
                  onChange={(e) => updateRoom(idx, "condition", e.target.value)}
                  className="border border-[#D4C5A0] rounded-lg px-2 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#30360E]/20"
                >
                ...
                <button
                  ...
                  className="text-[#7A7A6A] hover:text-red-600 transition-colors px-1"
                >
              ...
              <input
                ...
                className="w-full border border-[#D4C5A0] rounded-lg px-3 py-1.5 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-[#30360E]/20"
              />
            </div>
          ))}
        </div>
        ...
        <div className="space-y-1">
          <label className="text-xs font-medium text-[#4A4A3A]">Observações gerais</label>
          <textarea
            value={generalObs}
      ...
      {report && (
        <div className="bg-white border border-[#30360E]/30 rounded-xl p-6 space-y-4">
          <div className="flex items-center gap-2">
            ...
            <h2 className="font-serif text-lg text-[#30360E]">Laudo gerado</h2>
          </div>
      ...
      {history.length > 0 && (
        <div className="space-y-3">
          <h2 className="font-serif text-lg text-[#30360E]">Vistorias anteriores</h2>
          {history.map((h) => (
                ...
                <div className="px-5 pb-5 border-t border-[#D4C5A0]">
                  {h.general_observations && (
                    <div className="mt-3 mb-2">
                      <p className="text-xs text-[#7A7A6A] font-medium uppercase tracking-wide mb-1">Observações gerais</p>
                      <p className="text-sm text-[#4A4A3A]">{h.general_observations}</p>
                    </div>
                  )}
                  <p className="text-xs text-[#7A7A6A] font-medium uppercase tracking-wide mt-3 mb-1">Laudo</p>
