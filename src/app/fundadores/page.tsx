  ...
  return (
    <main className="min-h-screen bg-[#F5F0E0]">
      ...
      <section className="max-w-4xl mx-auto px-6 pt-16 pb-12 text-center space-y-5">
        <div className="inline-flex items-center gap-2 bg-[#787F56]/10 text-[#787F56] text-xs px-4 py-1.5 rounded-full font-medium">
          <Star className="w-3.5 h-3.5" />
          Programa Fundadores Moova
        </div>
        <h1 className="font-serif text-4xl lg:text-5xl text-[#30360E] leading-tight">
          Você não vai assinar um SaaS.<br />Vai construir um com a gente.
        </h1>
        <p className="text-[#4A4A3A] text-lg max-w-2xl mx-auto leading-relaxed">
      ...
      {/* What it means */}
      <section className="bg-[#30360E] py-12">
            ...
            {[
              ...
              <div key={title} className="space-y-3">
                <div className="w-10 h-10 bg-white/10 rounded-xl flex items-center justify-center mx-auto">
                  <Icon className="w-5 h-5 text-[#787F56]" />
                </div>
      ...
      <section className="max-w-4xl mx-auto px-6 py-16 space-y-10">
        <div className="text-center space-y-2">
          <p className="text-xs text-[#787F56] uppercase tracking-widest font-mono">As ondas</p>
          <h2 className="font-serif text-3xl text-[#30360E]">Dois grupos, critérios diferentes</h2>
        </div>
        <div className="grid sm:grid-cols-2 gap-6">
          {FUNDADORES.map((wave) => (
            <div
              key={wave.wave}
              className={`rounded-2xl border p-8 space-y-6 ${wave.highlight ? "border-[#787F56] bg-white shadow-lg" : "border-[#D4C5A0] bg-white"}`}
            >
              {wave.highlight && (
                <div className="text-[10px] font-bold text-[#787F56] uppercase tracking-widest">Hand-picked pelo fundador</div>
              )}
              <div>
                <h3 className="font-serif text-2xl text-[#30360E]">{wave.wave}</h3>
                <p className="text-sm text-[#7A7A6A] mt-1">{wave.slots} vagas · Seleção manual</p>
              </div>
              <ul className="space-y-2.5">
                {wave.benefits.map((b) => (
                  <li key={b} className="flex items-start gap-2 text-sm text-[#3A3A3A]">
                    <Star className="w-3.5 h-3.5 text-[#787F56] shrink-0 mt-0.5" />
                    {b}
                  </li>
                ))}
              </ul>
              <Link
                ...
                className={cn(
                  ...
                  wave.highlight ? "bg-[#787F56] hover:bg-[#9A6025] text-white" : "bg-[#30360E] hover:bg-[#4A5218] text-white"
                )}
              >
                Solicitar vaga
              </Link>
      ...
      {/* Origin Cora */}
      <section className="bg-white border-y border-[#D4C5A0] py-12">
        <div className="max-w-2xl mx-auto px-6 text-center space-y-5">
          <p className="text-xs text-[#787F56] uppercase tracking-widest font-mono">A origem da Nara</p>
          <blockquote className="font-serif text-lg text-[#30360E] leading-relaxed">
      ...
      <section className="max-w-4xl mx-auto px-6 py-16 text-center space-y-5">
        <h2 className="font-serif text-3xl text-[#30360E]">Ainda tem vagas?</h2>
        <p className="text-[#4A4A3A] max-w-md mx-auto">
          Manda um email ou solicite pelo formulário. A resposta sobre a seleção chega em até 48h úteis.
        </p>
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link href="/signup" className={cn(buttonVariants({ size: "lg" }), "bg-[#30360E] hover:bg-[#4A5218] text-white")}>
            Solicitar vaga de Fundador
          </Link>
          <a
            href="mailto:fundadores@moova.com.br"
            className={cn(buttonVariants({ size: "lg", variant: "outline" }), "border-[#D4C5A0] text-[#30360E]")}
          >
            Escrever direto
          </a>
        </div>
      ...
      {/* Footer */}
      <footer className="border-t border-[#D4C5A0] py-6">
        <div className="max-w-4xl mx-auto px-6 flex items-center justify-between text-xs text-[#7A7A6A]">
          <div className="flex items-center gap-2">
            <div className="w-1 h-3.5 bg-[#787F56] rounded-full" />
            <span className="font-serif text-[#30360E]">Moova</span>
          </div>
          <Link href="/" className="hover:text-[#30360E]">← Voltar ao site</Link>
        </div>
      </footer>
    </main>
  )
