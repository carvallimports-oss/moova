...
export default async function PortalPage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  return (
    <div className="min-h-screen bg-[#F5F0E0]">
      {/* Header */}
      <header className="bg-white border-b border-[#D4C5A0]">
        <div className="max-w-4xl mx-auto px-6 py-4 flex items-center justify-between">
          ...
          {whatsappHref && (
            <a
              ...
              className="flex items-center gap-2 text-sm font-medium text-white bg-[#30360E] rounded-lg px-4 py-2 hover:bg-[#20240A] transition-colors"
            >
      ...
      {/* Broker hero */}
      <section className="bg-white border-b border-[#D4C5A0]">
        <div className="max-w-4xl mx-auto px-6 py-10">
          <div className="flex items-start gap-6">
            <div className="w-20 h-20 rounded-full bg-[#30360E] flex items-center justify-center shrink-0">
              {userData.avatar_url ? (
            ...
            <div>
              ...
              <div className="flex flex-wrap items-center gap-3 mt-1">
                {location && (
                  <p className="flex items-center gap-1.5 text-sm text-[#7A7A6A]">
                    <MapPin className="w-3.5 h-3.5" />
                    {location}
                  </p>
                )}
                {userData.creci && (
                  <p className="flex items-center gap-1.5 text-xs text-[#4A4A3A] bg-[#E2D4B9] px-2 py-0.5 rounded-full">
                    <ShieldCheck className="w-3 h-3 text-[#787F56]" />
                    CRECI {userData.creci}
                  </p>
                )}
              </div>
              {userData.bio && (
                <p className="text-sm text-[#4A4A3A] mt-3 leading-relaxed max-w-xl">{userData.bio}</p>
              )}
              <div className="flex items-center gap-3 mt-4">
                {whatsappHref && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-2 text-sm text-[#30360E] border border-[#D4C5A0] bg-white rounded-lg px-3 py-2 hover:bg-[#E2D4B9] transition-colors"
                  >
      ...
      <main className="max-w-4xl mx-auto px-6 py-8">
        <h2 className="font-serif text-xl text-[#30360E] mb-6">
        ...
        {!properties?.length ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {properties.map((p) => {
              ...
              return (
                <div key={p.id} className="bg-white border border-[#D4C5A0] rounded-xl overflow-hidden hover:shadow-md transition-shadow">
                  <div className="h-44 bg-[#E2D4B9] relative overflow-hidden">
                    {coverUrl ? (
                      // eslint-disable-next-line @next/next/no-img-element
                      <img
                        src={coverUrl}
                      ...
                      <div className="w-full h-full flex items-center justify-center">
                        <Building2 className="w-8 h-8 text-[#787F56] opacity-40" />
                      </div>
                    )}
                    <div className="absolute top-2 left-2">
                      <span className="text-[10px] font-medium text-[#4A4A3A] bg-white/90 backdrop-blur-sm px-2 py-0.5 rounded-full uppercase tracking-wide">
                        {p.type ?? "Imóvel"}
                      </span>
                    </div>
                  </div>
                  <div className="p-4">
                    ...
                    {p.price && (
                      <p className="text-sm font-semibold text-[#30360E] mt-2">
                        R$ {p.price.toLocaleString("pt-BR")}
                      </p>
                    )}
                    {p.city && <p className="text-xs text-[#7A7A6A] mt-1">{p.city}</p>}
                    {whatsappHref && (
                      <a
                        ...
                        target="_blank"
                        rel="noopener noreferrer"
                        className="mt-3 w-full flex items-center justify-center gap-1.5 text-xs font-medium text-[#30360E] border border-[#D4C5A0] rounded-lg py-2 hover:bg-[#E2D4B9] transition-colors"
                      >
      ...
      {/* Footer */}
      <footer className="text-center py-8 text-xs text-[#7A7A6A] border-t border-[#D4C5A0] mt-8">
        <Link href="/" className="hover:text-[#30360E] transition-colors">
          Portal gerado pelo <span className="font-medium text-[#30360E]">Moova</span>
        </Link>
        {" · "}
        <Link href="/privacidade" className="hover:text-[#30360E] transition-colors">Privacidade</Link>
        {" · "}
        <Link href="/termos" className="hover:text-[#30360E] transition-colors">Termos</Link>
      </footer>
