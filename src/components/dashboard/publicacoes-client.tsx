...
function DraftCard({
  ...
  const platform = PLATFORM_LABELS[draft.platform] ?? { label: draft.platform, icon: Film as LucideIcon, color: "text-[#7A7A6A]" }
  ...
  return (
    <div className={cn(
      ...
      isPending   ? "border-[#787F56]/40 shadow-sm" : "border-[#D4C5A0]",
      isPublished && "border-[#30360E]/30 bg-[#30360E]/5",
    )}>
      {/* Header */}
      <div className="flex items-center justify-between">
        ...
        <span className={cn(
          ...
          isPending   && "bg-[#787F56]/10 text-[#787F56]",
          isApproved  && "bg-green-50 text-green-700",
          isRejected  && "bg-red-50 text-red-600",
          isPublished && "bg-[#30360E]/10 text-[#30360E]",
        )}>
      ...
      {editing ? (
        <div className="space-y-2">
          <textarea
            ...
            rows={4}
            className="w-full text-sm border border-[#D4C5A0] rounded-lg px-3 py-2 resize-none focus:outline-none focus:border-[#30360E]"
          />
          <div className="flex gap-2">
            <Button size="sm" onClick={handleSaveCaption} disabled={acting} className="bg-[#30360E] hover:bg-[#20240A] text-xs">
              {acting ? <Loader2 className="w-3 h-3 animate-spin" /> : "Salvar"}
            </Button>
            <Button size="sm" variant="outline" onClick={() => { setCaption(draft.caption ?? ""); setEditing(false) }} className="border-[#D4C5A0] text-xs">
              Cancelar
            </Button>
          </div>
        </div>
        ...
        <div>
          <p className="text-sm text-[#4A4A3A] leading-relaxed whitespace-pre-wrap line-clamp-5">
            {draft.caption || <span className="text-[#7A7A6A] italic">Sem legenda</span>}
          </p>
          {isPending && (
            <button onClick={() => setEditing(true)} className="text-xs text-[#787F56] hover:underline mt-1">
              Editar legenda
            </button>
          )}
        </div>
      ...
      {isPublished && draft.published_meta_id && (
        <div className="flex items-center gap-1.5 text-[11px] text-[#30360E]">
      ...
      {isPending && (
        <div className="flex gap-2 pt-1">
          <Button
            ...
            disabled={acting}
            className="flex-1 bg-[#30360E] hover:bg-[#20240A] text-xs gap-1"
          >
          ...
          <Button
            ...
            disabled={acting}
            className="flex-1 border-[#D4C5A0] text-[#7A7A6A] hover:text-red-600 hover:border-red-200 text-xs gap-1"
          >
            <X className="w-3 h-3" />
            Rejeitar
          </Button>
...
export function PublicacoesClient({
  ...
  return (
    <div className="space-y-4">
      ...
      {isMetaConnected && (
        <div className="bg-[#30360E]/5 border border-[#30360E]/20 rounded-xl px-4 py-3 flex items-center gap-2">
          <div className="w-2 h-2 rounded-full bg-green-500" />
          <p className="text-sm text-[#30360E]">
          ...
          <button
            ...
            className="ml-auto text-[11px] text-[#7A7A6A] hover:text-red-600 underline underline-offset-2"
          >
            Desconectar
          </button>
        </div>
      ...
      <div className="bg-[#E2D4B9] rounded-xl p-4 text-sm text-[#4A4A3A]">
        <span className="font-medium text-[#30360E]">Como funciona?</span>{" "}
      ...
      {/* Tabs */}
      <div className="flex flex-wrap gap-1 bg-[#E2D4B9] rounded-lg p-1 w-fit">
        {STATUS_TABS.map((t) => (
          <button
            key={t.key}
          ...
          >
            ...
            {t.key === "pending" && pendingCount > 0 && (
              <span className="ml-1.5 bg-[#787F56] text-white text-[9px] px-1.5 py-0.5 rounded-full font-bold">
                {pendingCount}
              </span>
            )}
            {t.key === "published" && publishedCount > 0 && (
              <span className="ml-1.5 bg-[#30360E] text-white text-[9px] px-1.5 py-0.5 rounded-full font-bold">
                {publishedCount}
              </span>
            )}
          </button>
