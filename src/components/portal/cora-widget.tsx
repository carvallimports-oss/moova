...
export function CoraWidget({ whatsappHref, brokerName }: CoraWidgetProps) {
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      {open && (
        <div className="bg-white border border-[#E0D8CE] rounded-2xl shadow-xl w-72 overflow-hidden animate-in fade-in slide-in-from-bottom-2 duration-200">
          {/* Widget header */}
          <div className="bg-[#2D4A3E] px-4 py-3 flex items-center gap-3">
            <div className="w-9 h-9 rounded-full bg-[#B87333] flex items-center justify-center shrink-0">
          ...
          <div className="p-4 space-y-3">
            ...
            <a
              ...
              className="w-full flex items-center justify-center gap-2 bg-[#2D4A3E] hover:bg-[#1e3329] text-white text-sm font-medium rounded-xl py-3 transition-colors"
            >
              <MessageCircle className="w-4 h-4" />
              Falar pelo WhatsApp
            </a>
            <p className="text-[10px] text-center text-[#8A8A8A]">
      ...
      <button
        ...
        className={cn(
          ...
          open
            ? "bg-[#5A5A5A] hover:bg-[#3A3A3A]"
            : "bg-[#2D4A3E] hover:bg-[#1e3329] hover:scale-105"
        )}
